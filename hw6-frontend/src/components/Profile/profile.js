...
export const Profile =  ({username,email,zipcode,dob,avatar,message,updateprof})=> {
    let newusername,newzipcode,newemail,newpassword,newavatar;
    { return (
        <div>
        <Navigation2Main/>
		{message===''?'':
	...
	<div className="col-md-4"><input type="password" size="20" ref={(node)=>{newpassword=node}}/></div>
			...
			<label className="btn btn-default btn-file">Upload Image<input type="file" style={{display: 'none'}} accept="image/*" onChange={(e) =>{newavatar=e.target.files[0]}}/></label>
        </form>
...
export default connect((state) => {
    ...
    (dispatch) => ({
	updateprof: (input,pw) => {dispatch(updateprofile(input,pw))}})
    )(Profile)
